...
#  after 'deploy', 'whenever:update_crontab'
  ...
  task :build do
    on roles(:app) do
      ...
      execute "cd #{current_path} && npm run build:prod" #npm run build:prod
  ...
  task :setup_nginx do
    on roles(:app) do
      execute " sudo ln -nfs #{current_path}/config/nginx-production.conf /etc/nginx/sites-enabled/#{fetch(:application)}"
    end
  end
end