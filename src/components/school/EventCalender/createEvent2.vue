<template>
  <div class="student-tab remove-padding" style="padding: 0 100px; margin-top: 100px">
    <e-dialog :title="'Custom audience selection '" v-model="dialog" width="60%" top="20vh" noEsc :gutter="20" hideDefaultFooter @cancel="dialog = false" @confirm="dialog = false">
      ...
      <el-row :gutter="20" style="margin-bottom: 15px" class="searchCard searchCard-2">
        <el-col :span="6" style="font-size: 18px; font-weight: bold; padding-left: 20px">
          ...
          <e-button :span="4" label="Deselect All" css="cancel_btn" @click="deselectCustom" />
        <el-col :span="4">
    ...
    <el-tabs class="floatTabHeader border-section" v-model="activeName" >
      ...
      <el-tab-pane label="Group" name="group">
        <el-row class="form-type-3 create-poll3-form" style="border-top: 1px solid #fcdfbd;margin-top: 20px;padding-bottom: 15px;">
  ...
  methods: {
    ...
    studentAudience() {
      this.students.map(x => {
        this.audience.push({
          user_id: x.id,
          audienceable_id: this.eventId,
          audienceable_type: 'Event',
          status: 'pending',
          user_type: x.role
        })
      })
