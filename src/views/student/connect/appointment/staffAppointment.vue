<template>
  <div>
    <es-container pageTitle="Staff Appointments" btnIcon="el-icon-plus" btnText="New Appointment" @btnClick="$router.push('/connect/createstaff-apointment/staff')" class="bg-black-5">
  ...
  methods:{
    ...
    getAppointments() {
      const query = `query($id: uuid) {
        appointments(where: {_and: [{created_by_id: {_eq: $id}}, {appointment_user_type: {_eq: "Staff"}}]}) {
          id
          title
          start_time
          status
