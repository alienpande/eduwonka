...
export default {
  ...
  methods: {
    ...
    getClassroom(){
      let type = this.$route.query.type ? 'appointment' : 'class';
    ...
    async getPeers(){
      ...
      this.socket.emit("GET_JOINED_PEERS", {}, (peers) => {
        ...
        for(const peer of peers){  
          // if(!this.sessionIds.includes(peer.peerSessionId) && peer.peerType !== 'Publisher'){
          //   this.consumeVideo(this.consumeTransport, peer)
          //   console.log('Connected Peers', peers)
          // }         
          // if(peer.peerType === this.socket.id){
          //   this.session_id = peer.peerSessionId
          // }
          if(peer.peerHandRaised && !this.handRisePeers.includes(peer.peerName)){
            this.handRisePeers.push(peer.peerName)
          }
        }
      });
    },
