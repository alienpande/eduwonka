<template>
  <e-container class="finance-view-task"  hideHeader pageTitle="View Task" :back="$url.FI_TASK">
    <e-steps :steps="steps" :active="activeTab" @active-tab="activeTabChange" style="width:300px"></e-steps>

    <view-task-1 v-if="activeTab == 1" @save="activeTab = 2" />
    <view-task-2 ref="tab2" v-if="activeTab == 2" @back="activeTab = 1" />
  </e-container>
</template>
<script>
...
export default {
  ...
  created() {
    console.log("in view task", this.taskid)
   // this.setAsideMenuCollapse(true);
