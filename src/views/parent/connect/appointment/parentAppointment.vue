<template>
  <div>
    <e-container pageTitle="Parent Appointments" >
  ...
  methods:{
    ...
    getAppointments() {
      const query = `query($id: uuid) {
        appointments(where: {appointment_user_type:{_eq:"Parent"}}) {
          id
          title
          start_time
          status
          location
          audience(where: {_and: [{user_id: {_eq: $id}}, {audienceable_type: {_eq: "Appointment"}}]}) {
            id
            status
            user {
              id
